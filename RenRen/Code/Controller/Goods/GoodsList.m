...
@interface GoodsList ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,DZNEmptyDataSetDelegate,DZNEmptyDataSetSource,UITableViewDataSource,UITableViewDelegate,GoodsCellDelegate>
...
-(void)viewDidAppear:(BOOL)animated{
    ...
    self.navigationItem.title =  @"一页快购";
    [self queryShopCar];
}
...
-(void)layoutUI{
    ...
    self.topView.backgroundColor = theme_default_color;
...
-(void)layoutConstraints{
    ...
    [self.view addConstraint:[NSLayoutConstraint constraintWithItem:self.collectionView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.view attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-45.f]];
...
-(void)queryData{
    ...
    NSDictionary* arg = @{@"ince":@"get_market_fruit_food_list",@"zoneid":self.Identity.location.circleID,@"pcate":[WMHelper integerConvertToString:self.categoryID],@"cate":[WMHelper integerConvertToString:self.subCategoryID],@"price":[WMHelper integerConvertToString:self.price],@"order":[WMHelper integerConvertToString:self.order], @"sid":self.storeID,@"page":[WMHelper integerConvertToString:self.netPage.pageIndex]};
