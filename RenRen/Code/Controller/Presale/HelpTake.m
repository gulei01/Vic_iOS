...
@interface HelpTake ()<UITextFieldDelegate,RandomBuyCarDelegate,RandomDeliverHelpDelegate>
...
@property(nonatomic,strong) UIImageView* iconTake;
@property(nonatomic,strong) UIButton* btnTake;
@property(nonatomic,strong) UILabel* labelTake;
@property(nonatomic,strong) UIImageView* takeArrow;
/**
 *  取货地址
 */
@property(nonatomic,strong) MAddress* takeAddress;

@property(nonatomic,strong) NSLayoutConstraint* takeConstraints;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(selectedTakeAddress:) name:NotificationSelectedRandomAddressTake object:nil];
...
-(void)layoutUI{
    ...
    [self.mainScroll addSubview:self.takeView];
    [self.takeView addSubview:self.iconTake];
    [self.takeView addSubview:self.btnTake];
    [self.takeView addSubview:self.labelTake];
    [self.takeView addSubview:self.takeArrow];
    ...
    [self layoutConstraints:@[
                              ...
                              @"H:|-defEdge-[nameView(==defWidth)]-defEdge-|",@"H:|-defEdge-[markView]-defEdge-|",@"H:|-defEdge-[takeView]-defEdge-|",
                              ...
                              @"H:|-leftEdge-[iconTake(==iconSize)]-leftEdge-[btnTake]-leftEdge-|",@"H:[iconTake]-itemSpace-[labelTake]-defEdge-[takeArrow(==arrowWidth)]-leftEdge-|",
                              @"V:|-iconTopEdge-[iconTake(==iconSize)]", @"V:|-defEdge-[btnTake]-defEdge-|",@"V:|-defEdge-[labelTake]-defEdge-|", @"V:|-arrowTopEdge-[takeArrow(==arrowHeight)]",
                              ...
                              @"H:|-leftEdge-[labelDelivery(==60)]-defEdge-[iconDelivery(==itemHeight)]-defEdge-[labelDeliveryPrice]-leftEdge-|",
                              @"V:|-topEdge-[labelDelivery]-topEdge-|", @"V:|-defEdge-[iconDelivery]-defEdge-|", @"V:|-topEdge-[labelDeliveryPrice]-topEdge-|",
                              
                              @"H:|-leftEdge-[labelVouchers(==titleWidth)]-leftEdge-[btnVouchers]-leftEdge-|", @"H:[labelVouchers]-defEdge-[labelVouchersPrice]-defEdge-[vouchersArrow(==arrowWidth)]-leftEdge-|",
                    ...
                    metrics:@{
                      ...
                      views:@{
                              ...
                              @"nameView":self.nameView,@"markView":self.markView,@"takeView":self.takeView,@"receiveView":self.receiveView,
                              ...
                              @"iconTake":self.iconTake, @"btnTake":self.btnTake,@"labelTake":self.labelTake, @"takeArrow":self.takeArrow,
    ...
    self.takeConstraints = [NSLayoutConstraint constraintWithItem:self.takeView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:itemHeight];
    [self.takeView addConstraint:self.takeConstraints];
...
-(void)loadConstraints{
    if(self.receiveAddress){
        if(self.receiveConstraints){
            ...
            self.receiveConstraints = [NSLayoutConstraint constraintWithItem:self.receiveView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:empty.height+14];
            [self.receiveView addConstraint:self.receiveConstraints];
        }
    }else{
        if(self.receiveConstraints){
    ...
    if(self.takeAddress){
        if(self.takeConstraints){
            [self.takeView removeConstraint:self.takeConstraints];
            CGSize empty = [self.labelTake intrinsicContentSize];
            self.takeConstraints = [NSLayoutConstraint constraintWithItem:self.takeView attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:empty.height+14];
            [self.takeView addConstraint:self.takeConstraints];
        }
    }else{
        if(self.takeConstraints){
...
-(void)submitRandomCar{
    if(self.Identity.userInfo.isLogin){
        if([self checkForm]){
            ...
            NSDictionary* arg = @{ @"ince": @"save_order_ince", @"uid":self.Identity.userInfo.userID, @"get_addr_id": self.takeAddress.rowID,
                                   @"send_addr_id":self.receiveAddress.rowID, @"xiao_fee":@(self.tipPrice), @"type": @"get", @"title":self.labelNameVal.text,
                                   @"weight": [self.infoDict objectForKey: @"weight"], @"price": @"", @"price_kind": [self.infoDict objectForKey: @"setting_id"],
                                   @"order_mark":self.txtMark.text,@"bonus_use":bonus_use, @"buy_addr":@{}
                                   };
...
-(IBAction)btnTakeTouch:(id)sender{
    if(self.Identity.userInfo.isLogin){
        ReceivesAddress* controller = [[ReceivesAddress alloc]initWithCategory:RandomAddressCategoryTake];
...
-(IBAction)goAgreementTouch:(id)sender{
    NSURL *URL = [NSURL URLWithString: @"http://wm.wm0530.com/Public/help/xieyi_get.html"];
    SVWebViewController *controller = [[SVWebViewController alloc] initWithURL:URL];
    [self.navigationController pushViewController:controller animated:YES];
}

-(IBAction)btnNameTouch:(id)sender{
    RandomSaleInfo* controller = [[RandomSaleInfo alloc]initWithType: @"get"];
    [self.navigationController pushViewController:controller animated:YES];
}
...
-(void)selectedTakeAddress:(NSNotification*)notification{
    self.takeAddress = [notification object];
    NSString* empty = [NSString stringWithFormat: @"%@ %@",self.takeAddress.userName,self.takeAddress.phoneNum];
    NSString* emptyAddress = [NSString stringWithFormat: @"%@ %@",self.takeAddress.mapAddress,self.takeAddress.mapNumber];
    NSString* str = [NSString stringWithFormat: @"%@\n%@",emptyAddress,empty];
    NSMutableAttributedString* attributeStr = [[NSMutableAttributedString alloc]initWithString:str];
    [attributeStr addAttributes:@{NSForegroundColorAttributeName:[UIColor blackColor]} range:NSMakeRange(0, str.length)];
    ...
    [paragraphStyle setLineSpacing:8];
    [attributeStr addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [str length])];
    self.labelTake.attributedText = attributeStr;
...
-(void)randomPayFinished:(NSNotification*)notification{
    ...
    /*
     self.takeAddress = nil;
     self.labelTake.text =  @"请选择取货地址(必填)";
     */
    self.tipPrice = 0;
...
-(void)calculateDelivery{
    if(self.takeAddress && self.receiveAddress){
        NetRepositories* respositorise = [[NetRepositories alloc]init];
        NSDictionary* arg = @{ @"ince": @"get_ship_fee", @"lat1":self.takeAddress.mapLat, @"lng1":self.takeAddress.mapLng, @"lat2":self.receiveAddress.mapLat, @"lng2":self.receiveAddress.mapLng};
...
-(BOOL)checkForm{
    if([WMHelper isNULLOrnil:self.infoDict]){
        ...
        return NO;
    }else if (!self.takeAddress){
        [self alertHUD: @"取货地址不能为空" delay:2];
        return NO;
    }else if (!self.iconAgreement.selected){
        [self alertHUD: @"请同意并接受《帮你取用户协议》" delay:2];
        return NO;
    }
    return YES;
}
...
-(UIImageView *)iconTake{
    if(!_iconTake){
        _iconTake = [[UIImageView alloc]init];
        [_iconTake setImage:[UIImage imageNamed: @"icon-qu"]];
    }
    return _iconTake;
}
...
-(UIButton *)btnTake{
    if(!_btnTake){
        _btnTake = [UIButton buttonWithType:UIButtonTypeCustom];
        [_btnTake addTarget:self action:@selector(btnTakeTouch:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _btnTake;
}
...
-(UILabel *)labelTake{
    if(!_labelTake){
        _labelTake = [[UILabel alloc]init];
        _labelTake.numberOfLines = 0;
        _labelTake.font = [self defTitleFont];
        _labelTake.textColor = [UIColor colorWithRed:159/255.f green:159/255.f blue:159/255.f alpha:1.0];
        _labelTake.text = @"请选择取货地址(必填)";
    }
    return _labelTake;
}

-(UIImageView *)takeArrow{
    if(!_takeArrow){
        _takeArrow = [[UIImageView alloc]init];
        [_takeArrow setImage:[UIImage imageNamed: @"icon-arrow-right"]];
    }
    return _takeArrow;
}
...
-(UILabel *)labelTipTitle{
    if(!_labelTipTitle){
        ...
        NSMutableAttributedString* attributeStr = [[NSMutableAttributedString alloc]initWithString:str];
        [attributeStr addAttributes:@{NSForegroundColorAttributeName:[UIColor redColor],NSFontAttributeName:[UIFont systemFontOfSize:14.f]} range:[str rangeOfString:first]];
