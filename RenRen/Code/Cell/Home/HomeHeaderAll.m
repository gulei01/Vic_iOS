...
@interface HomeHeaderAll ()<SDCycleScrollViewDelegate>
...
-(void)layoutUI{
    ...
    self.middleView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.txtSearch.frame)+5, SCREEN_WIDTH, 160+ADVHEIGHT*2+180)];
    ...
    self.bottomAdvView = [SDCycleScrollView cycleScrollViewWithFrame:CGRectMake(0,CGRectGetMaxY(self.typeView.frame), SCREEN_WIDTH, ADVHEIGHT) imageURLStringsGroup:nil];
    ...
    self.activityView = [[ActivityPart alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.bottomAdvView.frame), SCREEN_WIDTH, 180.f)];
...
{
    if(cycleScrollView == self.topAdvView){
        if(self.delegate && [self.delegate respondsToSelector:@selector(didSelectBannerPhoto:)]){
            [self.delegate didSelectBannerPhoto:index];
        }
    }else{
        if(self.delegate && [self.delegate respondsToSelector:@selector(didSelectSpecialPhoto:)]){
            [self.delegate didSelectSpecialPhoto:index];
        }
    }
}
...
-(void)loadNotice:(NSString *)notice{
    if(notice){
        _notice = notice;
        self.middleView.frame = CGRectMake(0, CGRectGetMaxY(self.txtSearch.frame)+5, SCREEN_WIDTH, 200+ADVHEIGHT*2+180);
        self.marqueeLabel.frame = CGRectMake(0, CGRectGetMaxY(self.topAdvView.frame), SCREEN_WIDTH, 40.f);
        self.typeView.frame =CGRectMake(0, CGRectGetMaxY(self.marqueeLabel.frame), SCREEN_WIDTH, 160);
        self.bottomAdvView.frame = CGRectMake(0,CGRectGetMaxY(self.typeView.frame), SCREEN_WIDTH, ADVHEIGHT);
        if([WMHelper isNULLOrnil:self.buyNow] && [WMHelper isNULLOrnil:self.fightGroup]){
            self.activityView.frame = CGRectMake(0, CGRectGetMaxY(self.bottomAdvView.frame), SCREEN_WIDTH, 180.f);
            self.activityView.hidden = NO;
        }else{
            self.activityView.frame = CGRectMake(0, CGRectGetMaxY(self.bottomAdvView.frame), SCREEN_WIDTH, 0.f);
