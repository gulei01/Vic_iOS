...
- (void)changeStarForegroundViewWithPoint:(CGPoint)point
{
    ...
    float fnum =p.x / (self.frame.size.width/5);
    
    NSString * str = [NSString stringWithFormat:@"%0.f",fnum];
    float score = [str floatValue];
   
    if(fnum>4.4){
        p.x = 5 * (self.frame.size.width/5);
    }else{
    p.x = (NSInteger)fnum * (self.frame.size.width/5);
    }
